...
class BraintreeSubmitView(EdxOrderPlacementMixin, BasePaymentSubmitView):
    ...
    The payment form should POST here. This view will handle creating the payment  at Braintree, creating an order,
    ...
    def form_valid(self, form):
        ...
        token = form_data['payment_method_nonce']
        ...
        try:
            ...
            logger.exception(
                'An error occurred while processing the Braintree payment for basket [%d].', basket.id)
            return JsonResponse({}, status=400)

        try:
            order = self.create_order(
                self.request, basket, billing_address=billing_address)
        except Exception:  # pylint: disable=broad-except
            logger.exception(
                'An error occurred while processing the Braintree payment for basket [%d].', basket.id)
            return JsonResponse({}, status=400)
